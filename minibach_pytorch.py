...
class Minibach(nn.Module):
    ...
    def loss_and_acc_on_epoch(self, batches_per_epoch, generator, train=True):
        ...
        for sample_id, next_element in tqdm(enumerate(islice(generator, batches_per_epoch))):
            ...
            loss = - (torch.log(output) * target).mean()
...
if __name__ == '__main__':
    ...
    model.load()
    model.train_model(batches_per_epoch=100, num_epochs=30,
                      plot=True)
    model.save()
    ...
    predictions = model(Variable(torch.FloatTensor(features[None, :]).cuda(), volatile=True))[0]
